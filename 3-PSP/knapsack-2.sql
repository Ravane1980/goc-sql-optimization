-- NECESSARY TABLES
-- CREATE TABLE objeto (id INTEGER, b1 INTEGER, b2 INTEGER, b3 INTEGER, b4 INTEGER, v INTEGER, w INTEGER)
-- SELECT * INTO global_solucion FROM objeto;
-- SELECT * INTO solucion FROM objeto;
-- SELECT * INTO new_solucion FROM objeto;
-- CREATE TABLE heuristica (fila INTEGER, v INTEGER, w INTEGER, vsuma INTEGER, wsuma INTEGER);
-- SELECT * INTO temp FROM heuristica;
-- CREATE TABLE pesos (fila INTEGER, w INTEGER);
-- CREATE TABLE valores (fila INTEGER, v INTEGER);

DROP PROCEDURE GetMaxKnapsack
...
BEGIN  
  ...
  INSERT INTO valores SELECT ROW_NUMBER() OVER (ORDER BY v DESC) as fila, v FROM objeto ORDER BY v DESC;
  
  INSERT INTO pesos SELECT ROW_NUMBER() OVER (ORDER BY w) as fila, w FROM objeto ORDER BY w;
  ...
  BEGIN
    ...
    INSERT INTO new_solucion SELECT DISTINCT 1 as id, (t1.b1 + t2.b1) as b1, (t1.b2 + t2.b2) as b2, (t1.b3 + t2.b3) as b3, (t1.b4 + t2.b4) as b4, (t1.v + t2.v) as v, (t1.w + t2.w) as w FROM
				...
				WHERE (t1.b1 & t2.b1) = 0
				      ...
				      AND (t1.b4 & t2.b4) = 0
				      AND t1.w + t2.w <= 400
				      AND (SELECT t1.v + t2.v + MAX(vsuma) FROM heuristica WHERE wsuma <= (400 - t1.w - t2.w)) > (SELECT MAX(v) FROM global_solucion);
